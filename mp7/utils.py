...
def hyperparameters():
    ...
    parser.add_argument('--trainsize', type=int, default=5000, help='number of training steps to take per iteration')
    parser.add_argument('--epsilon', type=float, default=0.5, help='exploration parameter')
    parser.add_argument('--gamma', type=float, default=0.9, help='discount reward factor. represents how confident a '
...
def rollout(env: gym.Env, policies: policies.QPolicy, episodes: int, epsilon: float, render: bool = False):
    ...
    for _ in range(episodes):
        ...
        while not done:
            ...
            action = np.random.choice(2,1,p=pi)[0]
            #print(action)
            ### Your Code Ends >>>
